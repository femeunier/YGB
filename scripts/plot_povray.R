...
#----- Paths. -----------------------------------------------------------------------------#
here           = "/home/femeunier/Documents/ED2.2/ED2/R-utils"   # Current directory.
there          = "/home/femeunier/Documents/projects/YGB/outputs/"    # Directory where analyses/history are
srcdir         = "/home/femeunier/Documents/ED2.2/ED2/R-utils" # Source  directory.
outroot        = "/home/femeunier/Documents/projects/YGB/Figures/" # Directory for figures
#------------------------------------------------------------------------------------------#
...
yearbeg        = 2015    # First year to consider
...
pov.nx.patch   <<-    5    # Number of sub-plots in each x transect
...
pov.x0         <<- rep( x     = -50 + seq(from=0,to=pov.nx.patch-1) * pov.patch.xmax
                        , times = pov.ny.patch)#end rep
pov.y0         <<- rep( x     = -50 + seq(from=0,to=pov.ny.patch-1) * pov.patch.ymax
                        , each  = pov.nx.patch)#end rep
...
for (place in myplaces){
  ...
  for (w in pcloop){
    ...
    dummy     = file.copy(from=file.path(srcdir,"povray_template_small.pov"),to=povscript)
    #------------------------------------------------------------------------------------#
