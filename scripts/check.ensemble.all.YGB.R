...
C.diurnal.ensemble <- OP.list[[3]] %>% filter(year > min(year)) %>% dplyr::select(hour,NEP,run)
Water.diurnal.ensemble <- OP.list[[1]] %>% filter(year > min(year)) %>% dplyr::select(hour,ET,run)
C.seasonal.ensemble <- OP.list[[4]] %>% filter(year > min(year)) %>% dplyr::select(month,NEP,run)
Water.seasonal.ensemble <- OP.list[[2]] %>% filter(year > min(year)) %>% dplyr::select(month,ET,run)
