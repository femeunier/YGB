...
Delta_dbh = 2 # to account for the fast jump in dbh in initial conditions
...
there   = '/home/femeunier/Documents/data/Yangambi/data/inventories/' # Census path
...
#------------------------------------------------------------------------------------------#
place      = "Yangambi"   # Name of the site, for output directory
census.csv = "Yangambi_census_MIX05.csv"  # Name of the csv file with the forest inventory data
iata       = "Yangambi"             # Tag for site (short name for output files)
identity   = "MIX05"         # Unique identification (in case you have multiple tests).
lon        = 0           # Longitude of the site
lat        =  24           # Latitude of the site
...
#------------------------------------------------------------------------------------------#
fast.soil.c   = 0.2     #  Litter layer              [kgC/m2]
...
outcohorts  = data.frame( time   = sprintf("%4.4i"  , rep(year.out,times=ncohorts))
                          ...
                          , dbh    = sprintf("%9.3f"  , pmax(0,census$dbh - Delta_dbh))
...
#
# plot(seq(1,Npatches),minH_T,type='p',col='green')
# # lines(seq(1,Npatches),minH_L,type='p',col='blue',ylim=c(0,1.1*max(minH_L)))
#
# #plot(census$dbh[census$pft==17],census$height[census$pft==17]) #,xlim=c(2,10))

system2("scp",paste(cssfile,"hpc:/data/gent/vo/000/gvo00074/felicien/IC/Yangambi/"))
system2("scp",paste(pssfile,"hpc:/data/gent/vo/000/gvo00074/felicien/IC/Yangambi/"))
