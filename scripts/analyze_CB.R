...
for (i in seq(1,nrow(df.mask))){
  ...
  if (nrow(details.file) > 0){
    ...
    file2check <- files.names[which(grepl("1999",files.names))]

    if (length(file2check) > 0){
      if (month(file.info(file2check)$ctime) == 7) next()
    }

    last.year = years.S.files[post.last]

    cdf <- data.frame()

    for (year in seq(1700,last.year,50)){
...
# scp /home/femeunier/Documents/projects/YGB/scripts/analyze_CB.R hpc:/data/gent/vo/000/gvo00074/felicien/R
